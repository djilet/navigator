	...
	function ShowHTML()
	{
		...
		switch ($request->GetProperty('View'))
		{
			case 'CategoryList':
                ...
                $content->SetVar("PagingHTML", $itemList->GetPagingAsHTML($url, $urlFirstPage));
...
//					$this->header["InsideModule"] = 1;
			...
			case 'ItemView':
				...
				if ($item->LoadByStaticPath($request))
				{
					...
					$request->SetProperty('CategoryID', $item->GetIntProperty('CategoryID'));
//					$this->header["InsideModule"] = 1;
...
*/
	...
	function ParseRequest()
	{
		...
		if ($categoryCount > 0)
		{
			...
			else if ($chunks == 1 || ($chunks == 2 && ($this->pathInsideModule[1] == "" || preg_match("/^([0-9]+)".str_replace(".", "\\.", HTML_EXTENSION)."$/", $this->pathInsideModule[1], $pageMatch))))
			{
				...
				{
                    preg_match(
                        "/^([0-9]+)".str_replace(".", "\\.", HTML_EXTENSION)."$/", 
                        $this->pathInsideModule[0], 
                        $pageMatch
                    );
                    
                    if (isset($pageMatch[1]) and $pageMatch[1] > 1) {
                        $request->SetProperty('View', 'CategoryList');
                        $request->SetProperty($itemList->GetPageParam(), $pageMatch[1]);
                    } 
                    elseif ($categoryInfo)
	...
	function ProcessHeader($module, Page $page = null)
	{
		$data = array();
