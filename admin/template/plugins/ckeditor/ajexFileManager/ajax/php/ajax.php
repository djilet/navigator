...
switch($mode) {
	...
	case 'QuickUpload':
		...
		if (0 == ($_FILES['upload']['error'])) {
            ...
            $filePath = $toDir . DIR_SEP . $fileName;
			if (move_uploaded_file($_FILES['upload']['tmp_name'], $filePath)) {
                ImageManager::compress($filePath, $filePath);
		...
		if (!empty($_FILES[$key])) {
			for ($i=-1, $iCount=count($_FILES[$key]['name']); ++$i<$iCount;) {
				...
				if (!in_array($ext, $cfg['deny'][$cfg['type']]) && in_array($ext, $cfg['allow'][$cfg['type']])) {
					...
					if (file_exists($cfg['root'] . $dir . $freeName)){
                        $filePath = $cfg['root'] . $dir . $freeName;
                        ImageManager::compress($filePath, $filePath);
                        chmod($filePath, $cfg['chmod']['file']);
                    }
				}
			}
		}
