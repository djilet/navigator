...
class Language extends BaseObject
{
	...
	function SetLanguageList($lngList, $setUTF8)
	{
		...
		if (count($this->_interfaceLanguageList) > 0)
		{
			...
			if ($setILang)
				setcookie("ILangCode", INTERFACE_LANGCODE, time()+60*60*24*30*COOKIE_EXPIRE, PROJECT_PATH, GetFromConfig('Domain', 'server'));
		}
		else
		{
	...
	function _LoadForPHP($module = null)
	{
		...
		else
		{
			for ($i = 0; $i < count($files); $i++)
			{
				if ($this->_LoadXML($files[$i]) and isset($this->_tags["Root"][1]))
				{
