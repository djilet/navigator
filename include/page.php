...
class Page extends LocalObject
{
	...
	function LoadByStaticPath($staticPath)
	{
		$query = "SELECT PageID, Path2Root, ".GetImageFields('MenuImage', count($this->params))."
			...
			FROM `page`
			WHERE WebsiteID=".intval(WEBSITE_ID)."
			AND StaticPath=".Connection::GetSQLString($staticPath);
		$this->LoadFromSQL($query);
	...
	function OpenParents()
	{
		...
		setcookie("expandedNodes", $expandedNodesJs, time()+60*60*24*30*COOKIE_EXPIRE, PROJECT_PATH, GetFromConfig('Domain', 'server'));
	}
