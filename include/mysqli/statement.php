...
class Statement
{
	...
	function Statement($dbLink, $resultType = MYSQLI_ASSOC, $errorLevel = E_USER_WARNING)
	{
		$this->_dbLink = $dbLink;
	...
	{
		$rs =& $this->Execute($query, is_string($field) ? MYSQLI_ASSOC : MYSQLI_BOTH);
		if (strtolower(get_class($rs)) == 'recordset')
		{
			$result = $rs->AllRows();
			$indexedResult = array();
    ...
    function FetchIndexedAssocList($query, $field = 0)
    {
        $rs = $this->Execute($query, MYSQLI_ASSOC);
        if (strtolower(get_class($rs)) == 'recordset') {
            $results = $rs->AllRows();
            $indexedResult = array();
            foreach ($results as $result) {
                if (array_key_exists($field, $result)) {
                    $indexedResult[$result[$field]] = $result;
                }
            }
            return $indexedResult;
        } elseif ($rs === false) {
	...
	function FetchRows($query, $resultType = null)
	{
		$rs = $this->Execute($query, $resultType);
		if (strtolower(get_class($rs)) == 'recordset')
			return $rs->AllSingleRows();
		elseif ($rs === false)
			return false;
		else
			return null;
