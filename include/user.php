...
class User extends LocalObject
{
	...
	{
		$query = "SELECT UserID, Email, Name, UserImage, UserImageConfig, Phone, Role, WebsiteID, Created, LastLogin, LastIP, PrivateKey 
					FROM `user`";
		...
		switch($authRole)
		{
			case INTEGRATOR:
				if ($forTemplate)
				{
					...
					$roles[] = array("Value" => ONLINEEVENT, "Title" => GetTranslation("role-".ONLINEEVENT));
					$roles[] = array("Value" => PARTNER, "Title" => GetTranslation("role-".PARTNER));
					$roles[] = array("Value" => API_SCANNER, "Title" => GetTranslation("role-".API_SCANNER));
					$roles[] = array("Value" => API_SCANNER_FULL, "Title" => GetTranslation("role-".API_SCANNER_FULL));
                    $roles[] = array("Value" => CONSULTANT, "Title" => GetTranslation("role-".CONSULTANT));
                    $roles[] = array("Value" => PROFTEST, "Title" => GetTranslation("role-".PROFTEST));
                    $roles[] = array("Value" => ROLE_UNIVERSITY, "Title" => GetTranslation("role-".ROLE_UNIVERSITY));
				}
				else
				{
				    $roles = array(INTEGRATOR, ADMINISTRATOR, ONLINEEVENT, PARTNER, API_SCANNER, API_SCANNER_FULL, CONSULTANT, PROFTEST, ROLE_UNIVERSITY);
				}
				break;
			case ADMINISTRATOR:
				if ($forTemplate)
				...
				{
				    $roles = array(ADMINISTRATOR, ONLINEEVENT, PARTNER, API_SCANNER, API_SCANNER_FULL, CONSULTANT, PROFTEST, ROLE_UNIVERSITY);
				}
				break;
	...
	{
	    $stmt = GetStatement();
	    $this->SetProperty("PrivateKey", md5(date("U").rand(1000, 9999)));
	    $stmt->Execute("UPDATE `user` SET PrivateKey=".$this->GetPropertyForSQL("PrivateKey")." WHERE UserID=".$this->GetIntProperty("UserID"));
	}
	
	function ValidateUserRequest(LocalObject $request, $roles)
	{
	    if($request->GetIntProperty("UserID") && $request->GetProperty("Sign"))
	    {
	        $stmt = GetStatement();
	        $privateKey = $stmt->FetchField("SELECT PrivateKey FROM `user` WHERE UserID=".$request->GetIntProperty("UserID")." AND Role IN (".implode(",", Connection::GetSQLArray($roles)).")");
	        if($privateKey)
	        {
	            ...
	            {
	                if($key != "Sign" && $key != "s" && !is_array($value))
	                    $paramStrings[] = $key."=".$value;
	            }
	            $correctSign = md5(implode("&", $paramStrings).$privateKey);
	            //print_r($correctSign);die();
