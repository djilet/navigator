...
class Module
{
	...
	function GetModuleList($selected = '', $adminOnly = false, $withConfig = false)
	{
		...
		foreach ($this->_moduleList as $k => $v)
		{
			...
			if (($adminOnly == true && file_exists($file)) || $adminOnly == false)
			{
				...
				if(isset($data['NoPages']) && $data['NoPages'])
					$mLinks[$i]['NoPages'] = true; 
				$i++;
			}
		}
		return $mLinks;
	...
	function LoadForHeader($folder, $template = null)
	{
		if (!$this->_ValidateModule($folder))
			return false;
		...
		{
			return $m->ProcessHeader($folder, $template);
		}
	}
	...
	function LoadLinks($folder, $subDomain = null)
	{
		if (!$this->_ValidateModule($folder))
			return false;
		...
		if (!method_exists($m, "ProcessLinks"))
		{
			return false;
		}
		else
		{
			return $m->ProcessLinks($folder, $subDomain);
		}
	}
	
	function ProcessApiRequest($folder, $method, $chunks, $request)
	{
	    if (!$this->_ValidateModule($folder))
	        return false;
	    ...
	    if (!method_exists($m, "ProcessApi"))
	    {
	        return false;
	    }
	    else
	    {
	        return $m->ProcessApi($folder, $method, $chunks, $request);
	    }
	}
	...
	public static function getAdminModuleUrl($module){
        $userPage = new PageList();
        $userPage->LoadPageListForModule($module);
        if (isset($userPage->_items[0]['PageID'])){
            $url = ADMIN_PATH . 'module.php?load=' . $module . '&PageID=' . $userPage->_items[0]['PageID'];
            return $url;
        }
        return false;
    }
