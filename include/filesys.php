...
class FileSys extends LocalObject
{
	function Upload($paramName, $toDir, $saveOriginalFileName = false, $acceptMimeTypes = array('image/png', 'image/x-png', 'image/gif', 'image/jpeg', 'image/pjpeg'))
	{
		...
		for ($i = 0; $i < count($files); $i++)
		{
			...
			{
				$startFileName = RuToStaticPath($files[$i]['name']); 
				$fileName = $startFileName;
				$f = 1;
				while (file_exists($toDir.$fileName) && $fileName !== $saveOriginalFileName)
				{
					$fileName = substr($startFileName, 0, strlen($startFileName) - strlen($extension[1]) - 1)."_(".$f.").".$extension[1];
					$f++;
				}
			}
			else
