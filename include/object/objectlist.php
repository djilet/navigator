...
class ObjectList extends CommonObject
{
	...
	function GetPagingAsArray($url, $urlFirstPage = null, $suffix = '')
	{
		...
		if (isset($urlComponents["path"]) and !strstr($urlComponents["path"], "[[".$this->GetPageParam()."]]"))
		{
		...
		if (count($this->_pagesArray) > 0)
		{
			foreach ($this->_pagesArray as $k => $v)
			{
				...
				$this->_pagesArray[$k]["URL"] = $newUrl.$suffix;
			}
		}
	...
	function GetPagingAsHTML($url, $urlFirstPage = null, $suffix = '')
	{
		$this->GetPagingAsArray($url, $urlFirstPage, $suffix);
