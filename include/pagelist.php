...
class PageList extends LocalObjectList
{
	...
	function GetPageList()
	{
		...
		for ($i = 0; $i < count($items); $i++)
		{
			...
			if ($parentID > 0)
			{
				...
				if (isset($icon['ColorA'])) {
                    $items[$i]['ColorA'] =  $icon['ColorA'];
                }
				if (isset($icon['ColorI'])) {
	...
	function LoadPageListForSelection($template, $selectedID=0)
	{
		$templateQuery = array();
		if(is_array($template))
			for($i=0; $i<count($template); $i++)
				$templateQuery[] = "Template=".Connection::GetSQLString($template[$i]);
		else 
			$templateQuery[] = "Template=".Connection::GetSQLString($template);
		
		$query = "SELECT PageID, Title AS Title, (CASE PageID WHEN ".intval($selectedID)." THEN 1 ELSE 0 END) as Selected, StaticPath
			FROM `page`
			WHERE WebsiteID=".intval(WEBSITE_ID)."
			AND (".implode(" OR ", $templateQuery).")
			AND LanguageCode=".Connection::GetSQLString(DATA_LANGCODE)."
		ORDER BY Title ASC";
		$this->LoadFromSQL($query);
	}
	...
	function GetPageTree($pageID = null, $asList = false, $defineCurrent = true, $active = 'Y')
	{
		...
		array_pop($urlParts);
		$url2 = implode('/', $urlParts);
		array_pop($urlParts);
		$url3 = implode('/', $urlParts);
		...
		$query = "SELECT SUBSTRING(UCASE(Title), 1, 1) AS FirstChar,
				...
				".(count($parentIDs) ? "IF (PageID IN (".implode(",", $parentIDs).") OR REPLACE(Link, '<P_T_R>', '".PROJECT_PATH."')=".Connection::GetSQLString($url)." OR REPLACE(Link, '<P_T_R>', '".PROJECT_PATH."')=".Connection::GetSQLString($url2)." OR REPLACE(Link, '<P_T_R>', '".PROJECT_PATH."')=".Connection::GetSQLString($url3).", 1, 0) AS Opened " : " 0 AS Opened").",
